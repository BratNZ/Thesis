...
def gyro_cal():
    ...
    [icm.gyro for ii in range(0,cal_size)] # clear buffer between readings
    mpu_array = [] # imu array for gyro vals
    gyro_offsets = [0.0,0.0,0.0] # gyro offset vector
    while True:
        try:
            _,_,_,wx,wy,wz = icm.gyro # read and convert mpu6050 data
        except:
            continue
...
#
def get_accel():
    ax,ay,az,_,_,_ = icm.acceleration # read and convert accel data
    return ax,ay,az
...
def accel_cal():
    ...
    for qq,ax_qq in enumerate(axis_vec):
        ...
        for direc_ii,direc in enumerate(cal_directions):
            ...
            [icm.acceleration for ii in range(0,cal_size)] # clear buffer between readings
            mpu_array = []
...
def mpu_plot_test():
    ...
    while True:
        try:
            ax,ay,az = icm.acceleration # read and convert mpu6050 data
